...
Concept: Building on context.WithCancel, Go provides context.WithTimeout and context.WithDeadline for automatic cancellation after a duration or at a
specific time. This is essential for preventing operations from hanging indefinitely.
...
Key learning: Always set timeouts on external calls (HTTP, database, etc.) to prevent goroutine leaks and hung processes. The defer cancel() pattern
ensures cleanup even if the operation completes before timeout.
